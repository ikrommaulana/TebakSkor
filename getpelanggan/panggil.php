...
<table cellspacing='0' id="testTable" class="table table-hover" summary="Code page support in different versions of MS Windows." rules="groups" frame="hsides">
	<tr class="success"><th class="success">id_users</th><th>nama</th><th>email</th><th>mobile</th><th>phone</th><th>address</th><th>status</th><th>propinsi</th><th>kota</th><th>kecamatan</th><th>kelurahan</th><th>rw</th><th>rt</th><th>joined_date</th></tr><!-- Table Header -->
...
if(isset($_GET['dari'])){
		$from=$_GET['dari'];
		$until=$_GET['sampai'];
		$ambil = mysql_query("SELECT * FROM register_user WHERE nama != '' and created_date>='$from' and created_date<='$until' ORDER BY created_date DESC ");
		echo "<br/><center>Pencarian Data dari tanggal  <strong>".$from."</strong>  Sampai tanggal  <strong>".$until."</strong></center><br/>";
		} else {
		$now = date("Y-m-d");
		$ambil = mysql_query("SELECT * FROM register_user WHERE nama != '' and created_date='$now' ORDER BY created_date DESC ");
		}
while ($row=mysql_fetch_object($ambil))
{
$nama = stripslashes($row->nama);
$joined_date = stripslashes($row->created_date);
...
$kecamatan =  stripslashes($row3->district_name);
...
/* echo $n; */
	...
	if ($n % 2 == 0)
  {
    echo "<tr><td>$id_users</td><td>$nama</td><td>$email</td><td>$mobile</td><td>$phone</td><td>$address</td><td>$status</td><td>$propinsi</td><td>$kota</td><td>$kecamatan</td><td>$kelurahan</td><td>$rw</td><td>$rt</td><td>$joined_date</td></tr>";

  }
  else
  {
    echo "<tr class='even'><td>$id_users</td><td>$nama</td><td>$email</td><td>$mobile</td><td>$phone</td><td>$address</td><td>$status</td><td>$propinsi</td><td>$kota</td><td>$kecamatan</td><td>$kelurahan</td><td>$rw</td><td>$rt</td><td>$joined_date</td></tr>";
  }
