...
class Match extends CI_Controller {
    ...
    function update($id) {
      ...
      if($this->input->post('submit')){
            ...
            $true = $this->mmatch->get_match($id);
			foreach ($true as $true){
				$match 		= $true->id_match;
				$guess 		= $true->id_guess;
				$user  		= $true->id_user;
				$username 	= $true->username;
				$guess_home	= $true->guess_home;
				$guess_away = $true->guess_home;
				
				if(($guess_home==$home_score)&&($guess_away==$away_score)){
					$point = "1";
				} else {
					$point = "0";
				}
			$this->mmatch->insert_point($match,$guess,$user,$username,$point);
			}
			redirect('root/match/');
        }
